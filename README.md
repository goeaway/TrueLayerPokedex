...
This was done with the `IDistributedCache` interface, implemented with an in memory distributed cache. This was a perfect implementation for this project, but in a real world scenario, you'd want to use an actually distributed cache running separately from the API. I chose this interface because it was supplied by dotnet itself, and has other implementations, such as the Redis one. Changing to an actually distributed version would be simple, only requiring me to change the DI registration in the Startup.cs to register the Redis version. I'd also consider updating the API to return some headers to indicate the cache was used and when it expires.

#### Deploy to multiple instances

Currently, the deployment of this application is a single dotnet API instance. This is fine for this challenge, but in a real world scenario a single instance would not hold up well against high traffic usage. To handle higher demand, I could deploy this as a number of instances protected by a load balancer application. You could also utilise Kubernetes to manage the addition or subtraction of instances to handle demand fluctuations.
...
Currently, the system is quite brittle when dealing with third party APIs. For example, if the PokeApi or translation API returned a 500 error momentarily, the system would handle this as an error and return a response, but in a production app you may want more resiliency. Here I would make use of the [Polly](https://github.com/App-vNext/Polly) nuget package to allow those interactions to be retried or go to a fallback.

#### More versatile logging sink

In production, I'd add a logging sink that could log out to a DB or a service like AWS CloudWatch. Currently the app only logs to the console.

#### Rate Limiting

In production, I'd add rate limiting functionality. Public APIs, such as this, can be easily abused if they don't restrict usage. It was not necessary for this challenge, but it would be valuable in a real world scenario. I could do this by using [this nuget package](https://github.com/stefanprodan/AspNetCoreRateLimit) which adds middleware to the request pipeline that can reject requests early from IPs if they violate the rate limit. 
...
When the project gets data from the PokeApi, it is possible this might fail somehow, the most common issue might be that the pokemon name was not recognised. In this case the PokeApi returns a 404. When the project sees this error it will return a response of the same status code to its caller. This could be improved. Imagine a scenario where we hit some rate limit in the PokeApi, it would probably return a 429 response, but when the caller of our API sees this response they might think they've violated our rate limit, when in reality the API has violated the PokeApi's one. This might be confusing and could be improved by handling the response from the PokeApi differently for certain responses. I didn't do this in this project because I was apprehensive about handling multiple cases and concerned about missing one, so I err'd on the side of simplicity and returned the PokeApi's response directly. 
...
This project contains models, DTOs, options, and "low" level interfaces to be used in various different parts of the application. This project is quite thin, only containing a few models and an `IUtcNowProvider`, an interface that is more mockable than using `DateTime.Now` directly
...
You can find the handlers for each endpoint in this project, in the `Queries` directory.
...
Once I was happy with both endpoints, I decided to implement the caching. This took a few different shapes before I was happy with the implementation. I started by using the `IDistributedCache` in both the endpoint handlers, but didn't like this because there was a fair bit of duplicate code between them. I experimented with a few other ways, such as trying a MediatR pipeline behaviour, and then trying to abstract the `IDistributedCache` away from the handlers. I finally settled on a version that made use of the [Scrutor](https://github.com/khellang/Scrutor) library, which allowed me to easily utilise the decorator pattern and remove all caching code from the handlers, and rely instead upon the DI container to serve the correct classes when needed.
